...
def pair():
    global is_suspended, oled
    is_suspended = True
    sensor_id = os.getenv("ID")
    image = Image.new("1", (WIDTH, HEIGHT))
    draw = ImageDraw.Draw(image)
    font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 16)
    draw.text((10, 0), str(sensor_id), font=font, fill=255)
    oled.image(image)
    oled.show()
    time.sleep(10)
    is_suspended = False
...
def display_message(oled_device, message="Breezy starting please wait...."):
    image = Image.new("1", (WIDTH, HEIGHT))
    draw = ImageDraw.Draw(image)
...
def main():
    ...
    while True:
        if sensor.data_ready() and not is_suspended:
            data = sensor.read_alg_result_data()
            eco2 = (data[0] << 8) | data[1]
            tvoc = (data[2] << 8) | data[3]
            sensor_data = {"eco2": eco2, "tvoc": tvoc}
            ...
            requests.post(AWS_SERVER_URL, json=sensor_data)
        time.sleep(10)
    GPIO.cleanup()
